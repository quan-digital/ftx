...
class FtxClient:
    ...
    def modify_order(
        ...
        assert (price is None) or (
            size is None
        ), "Must modify price or size of order"
        path = (
    ...
    def get_fills(
        ...
        return self._get(
            "fills", {"start_time": start_time, "end_time": end_time}
        )
    ...
    def get_deposit_address(
        self, ticker: str, method: Optional[str] = None
    ) -> dict:
    ...
    def get_position(self, name: str, show_avg_price: bool = False) -> dict:
        return next(
            filter(
                lambda x: x["future"] == name,
                self.get_positions(show_avg_price),
            ),
            None,
        )
    ...
    def delete_subaccounts(self, nickname: Optional[str] = None) -> List[dict]:
        assert (nickname is not None) or (
            self._subaccount_name is not None
        ), "SubAccount not set"
    ...
    def get_subaccounts_balance(self, nickname=None) -> List[dict]:
        ...
        subaccount = nickname or self._subaccount_name
        return self._get(
            f"subaccounts/{subaccount}/balances", {"nickname": subaccount}
        )
    ...
    def request_quote(self, fromCoin, toCoin, size) -> List[dict]:
        return self._post(
            "otc/quotes",
            {"fromCoin": fromCoin, "toCoin": toCoin, "size": size},
        )
    ...
    def request_withdrawal(
        ...
        return self._post(
            "wallet/withdrawals",
            {"coin": coin, "size": size, "address": address},
        )
    ...
    def get_all_trades(
        ...
        while True:
            ...
            if len(response) == 0:
                break
            end_time = min(
                parse_datetime(t["time"]) for t in response
            ).timestamp()
    ...
    def get_future_stats(self, future_name) -> List[dict]:
        return self._get(
            f"futures/{future_name}/stats", {"future_name": future_name}
        )
    ...
    def get_funding_rates(
        ...
        return self._get(
            "funding_rates",
            dict(future=future, start_time=start_time, end_time=end_time),
        )
